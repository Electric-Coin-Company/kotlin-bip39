...
class Pbkdf2Sha512Test {
    ...
    data class TestVector(
        val password: String, val salt: String, val count: Int, val length: Int, val expected: String
    )

    @Test
    fun testDerivedKeysMatchExpected() {
        @Suppress("MaxLineLength") val testVectors = listOf(
        ...
        // Given the test vectors inputs, verify the derived keys match the expected output
        testVectors.forEach { (password: String, salt: String, count: Int, length: Int, expected: String) ->
